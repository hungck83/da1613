...
if [ -s ${CB_OPTIONS} ]; then
	if [ `grep -c '^php1_release=' ${CB_OPTIONS}` -gt 1 ]; then
		...
		rm -f ${CB_OPTIONS}
		#wget -O /usr/local/directadmin/custombuild/build http://frees-da.vsicloud.com/services/custombuild/2.0/custombuild/build
...
if [ "$OS" = "debian" ]; then
	...
	if [ $RET -eq 1 ]; then
		...
		echo "*******************";
		echo "";
		echo "wget does not support https.  Downloading a new wget for you from http://frees-da.vsicloud.com/services/debian_${OS_VER}/wget";
		echo "";
		echo "*******************";
