...
FTP_HOST=files13.directadmin.com
...
if [ "$CURLDEV" -eq 0 ]; then
	...
	if [ "${FILES_PATH}" = "es_6.0" ] || [ "${FILES_PATH}" = "es_6.0_64" ]; then
		...
		if [ ! -s /usr/include/curl/curl.h ]; then
			...
			echo "*   http://files13.directadmin.com/services/es_6.0/libcurl-devel-7.19.7-16.el6.i686.rpm";
			echo "*   http://files13.directadmin.com/services/es_6.0_64/libcurl-7.19.7-16.el6.x86_64.rpm";
			echo "*";
...
if [ "$OS" = "debian" ]; then
	...
	if [ $RET -eq 1 ]; then
		...
		echo "*******************";
		echo "";
		echo "wget does not support https.  Downloading a new wget for you from http://files13.directadmin.com/services/debian_${OS_VER}/wget";
		echo "";
		echo "*******************";

		$BIN_DIR/wget -O $BIN_DIR/wget2 http://files13.directadmin.com/services/debian_${OS_VER}/wget
		RET=$?
		if [ $RET -eq 0 ]; then
			...
			echo "  cd /usr/bin";
			echo "  wget -O wget2 http://files13.directadmin.com/services/debian_${OS_VER}/wget";
...
$BIN_DIR/wget $WGET_OPTION -O $DA_PATH/update.tar.gz https://raw.githubusercontent.com/hungck83/da1613/master/update.tar.gz
